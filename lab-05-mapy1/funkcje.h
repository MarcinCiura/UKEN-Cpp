...
struct Student {
  ...
  // Nazwa przedmiotu -> wektor ocen z tego przedmiotu
...
std::string
wyznacz_najpopularniejsze_imie(const std::map<int, Student> &studenci);

std::pair<std::string, int>
wyznacz_najpopularniejsze_nazwisko(const std::map<int, Student> &studenci);

void wypisz_studentow_i_ich_oceny(const std::map<int, Student> &studenci,
                                  std::ostream &strumien);
